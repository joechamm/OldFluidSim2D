...
void InitFBO( void )
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_RECTANGLE, dens_tex[0], 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_RECTANGLE, press_tex[0], 0);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_RECTANGLE, dens_tex[1], 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_RECTANGLE, press_tex[1], 0);
...
void InitShaders( void )
{
	...
	prog_render.AddUniform("DensityTexture");
	prog_render.AddUniform("GridWidth");
	prog_render.AddUniform("GridHeight");
	prog_handle = prog_render.GetProgram();
	densSubIdx = glGetSubroutineIndex(prog_handle, GL_FRAGMENT_SHADER, "RenderDens");
	...
	prog_advect.AddUniform("DensityTexture");
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'r':
		...
		break;
	case 's':
		current_subroutine = (current_subroutine + 1) % 3;
...
void InitGlobals( void )
{
	...
	advect_drawBuffs[2] = GL_COLOR_ATTACHMENT2;
	...
	press_drawBuffs[0] = GL_COLOR_ATTACHMENT3;
...
void DrawSquare( void )
{
	...
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_RECTANGLE, dens_tex[current_fbo]);
	...
	glUniform1i(prog_render("DensityTexture"), 2);
	...
	else if(current_subroutine == 2)
	{
		glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &densSubIdx);
	}
